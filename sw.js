// TODO: read this sw.js (taken from offline-wikipedia repo of jakearchibald) and modify this as required..
      ...
      return cache
        ...
        .then((response) => {
          ...
          return response.text().then((text) => {
            ...
            const articleEnd = text.indexOf(
              '<div class="background-load-offer card">',
            );
            return Promise.all([
              cache.put(
                "/shell-start.html",
                new Response(text.slice(0, headerEnd), response),
              ),
              cache.put(
                "/shell-end.html",
                new Response(text.slice(articleEnd), response),
              ),
            ]);
          });
        });
...
function streamArticle(url) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const contentFetch = fetch(contentURL).catch(
        () => new Response("Failed, soz"),
      );
  ...
  event.waitUntil(
    storage
      ...
      .then((toCache) => {
        ...
        return Promise.all(
          toCache.map(async (articleName) => {
            var article = await wikipedia.article(articleName);
            await article.cache();
            registration.showNotification(
              (await article.meta).title + " ready!",
              {
